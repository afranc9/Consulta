...
function CargarNodoPrincipal() {
    ...
    ref.orderByKey().on("child_added", function (snapshot) {
        ...
        var divchat =
            '<a href="javascript:cargarConversacion(\'' + id + '\',\'' + snapshot.val().nickName + '\');">' +
...
window.onload = function () {
    ...
    $("#btnInitChat").click(function () {
        $("#divOpenChat").attr("style", "display:none;");
        $("#messages-card-container").attr("style", "");

        $("#divChat").attr("style", "displat:block");

        var nickName = $("#footerNickName").val();
        var edad = $("#footerAge").val();
        ...
        var fecha = new Date();
        var codeConversation = fecha.toISOString().slice(0, 10).replace(/-/g, "") + "" + fecha.getHours() + "" + fecha.getMinutes() + "" + fecha.getSeconds();
        var datenow = fecha.toISOString().slice(0, 10) + " " + fecha.getHours() + ":" + fecha.getMinutes() + ":" + fecha.getSeconds();

        window.friendlyChat = new FriendlyChat();

        //Creamos el nodo con los datos de la conversacion

        firebase.database().ref('Conversaciones/' + codeConversation).set({
            nickName: nickName,
            edad: edad,
            pais: pais,
            sexo: sexo,
        ...
        firebase.database().ref('Conversaciones/' + codeConversation).push({
            //name: currentUser.displayName,
            name: currentName,
            idconversacion: codeConversation,
            text: consulta,
            fechahora: datenow
            //photoUrl: currentUser.photoURL || '/images/profile_placeholder.png'
        }).then(function () {
